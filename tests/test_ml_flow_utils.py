...
def mock_mlflow_env(monkeypatch):
    ...
    monkeypatch.setenv(
        "HYDRA_CONFIG_DIR", "artifacts/test_group"
    )  # Mock where configs are
...
def test_mlflow_run_success(
    MockPath, MockPandas, MockOmegaConf, mock_mlflow_module, mock_mlflow_env, temp_cwd
):
    # --- Setup Mocks ---
    ...
    # Mock config loading
    mock_config_content = OmegaConf.create(
    ...
    MockPandas.json_normalize.return_value.to_dict.return_value = [
        {"param1": "value1", "nested.param2": 10}
    ]

    # Mock Path for .pipeline_id, config file, and log file
    mock_pipeline_id_path_instance = MagicMock(spec=Path)
    mock_pipeline_id_path_instance.exists.return_value = (
        False  # Test creating .pipeline_id
    )
    mock_pipeline_id_path_instance.read_text.return_value = (
        "old_parent_run_id"  # If it existed
    )

    mock_config_path_instance = MagicMock(spec=Path)
    mock_config_path_instance.exists.return_value = True
    mock_config_path_instance.as_posix.return_value = (
        "artifacts/test_group/test_stage.yaml"
    )
    mock_config_path_instance.read_text.return_value = (
        "artifacts/test_group/test_stage.yaml"
    )
    ...
    mock_hydra_config_get.get.return_value.runtime.output_dir = Path(
        temp_cwd / "artifacts/test_group/test_stage"
    )
    mock_log_path_instance = MagicMock(spec=Path)
    mock_log_path_instance.as_posix.return_value = str(
        temp_cwd / "artifacts/test_group/test_stage/run.log"
    )
    ...
    mock_mlflow_module.log_artifact.assert_any_call(
        str(temp_cwd / "artifacts/test_group/test_stage/run.log")
    )

    # Check .pipeline_id was written
    mock_pipeline_id_path_instance.write_text.assert_called_once_with(
        mock_parent_run.info.run_id + "\n"
    )


@patch("zenflow.mlflow_utils.mlflow")
@patch(
    "zenflow.mlflow_utils.OmegaConf"
)  # Mock less for failure path, focus on exception
...
def test_mlflow_run_failure(
    ...
    mock_hydra_config_get = MagicMock()
    mock_hydra_config_get.get.return_value.runtime.output_dir = str(
        temp_cwd / "artifacts/test_group/test_stage"
    )
    ...
    with pytest.raises(ValueError, match="Simulated failure"):
        with patch(
            ...
            decorated_function(
                "fail"
            )  # This argument makes sample_stage_function raise an error
    ...
    mock_mlflow_module.log_artifact.assert_any_call(
        str(temp_cwd / expected_log_path_str)
    )
