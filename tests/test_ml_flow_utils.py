...
def test_mlflow_run_success(
    MockPath, MockPandas, MockOmegaConf, mock_mlflow_module,
    mock_mlflow_env, temp_cwd
):
    # --- Setup Mocks ---
    ...
    MockOmegaConf.to_container.return_value = {"param1": "value1", "nested.param2": 10} # Flattened
...
def test_mlflow_run_failure(
    MockPath, MockOmegaConf, mock_mlflow_module,
    mock_mlflow_env, temp_cwd # Use the env fixture
):
    mock_parent_run = MagicMock()
    mock_child_run = MagicMock()
    mock_mlflow_module.start_run.return_value.__enter__.side_effect = [mock_parent_run, mock_child_run]
    ...
    with pytest.raises(ValueError, match="Simulated failure"):
         with patch("zenflow.mlflow_utils.hydra.core.hydra_config.HydraConfig", mock_hydra_config_get):
